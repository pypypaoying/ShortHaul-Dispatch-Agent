...
| 使用场景 | 推荐选择 | 说明 |
| --- | --- | --- |
| 常见中文业务表、字段名相对清楚 | DeepSeek `deepseek-chat` | 成本较低，中文字段理解较稳，适合作为默认首选。 |
| 阿里云/DashScope 环境或表格字段较规整 | 通义千问 `qwen-plus` | OpenAI-compatible endpoint 易接入，表格摘要对齐表现通常稳定。 |
| 表格特别乱、备注很多、跨文件关系复杂 | 更强的长上下文模型 | 优先关注 JSON 稳定性和长上下文能力，而不是只看推理榜单。 |
...
当前项目环境中已验证的可用配置：DeepSeek `deepseek-chat` 与通义千问 `qwen-plus` 都能在杂乱表样例上完成对齐和求解；DeepSeek 在该样例上延迟更低，因此 UI 中推荐优先试用 DeepSeek，再用脚本在自己的 API Key 和真实数据上复测。
...
3. 编辑自然语言调度需求。
4. 从供应商预设中选择 DeepSeek、通义千问、OpenAI、Kimi、GLM、OpenRouter 或自定义 endpoint。
