...
- Web UI：支持上传业务系统导出的 Excel/CSV/JSON 或粘贴表格文本，由数据接入 Agent 自动对齐模型输入。
...
1. 启动 Web UI。
2. 上传业务系统导出的 Excel/CSV/JSON，或直接粘贴表格文本。
3. 如数据字段不规整，在页面中配置数据接入 Agent 的 OpenAI-compatible API Key、Base URL 和模型。
...
$env:SHORT_HAUL_INGESTION_BASE_URL="https://api.openai.com/v1"
$env:SHORT_HAUL_INGESTION_MODEL="gpt-4.1-mini"
```
...
用户可以接入自己的微调数据接入模型；系统只需要一个兼容 OpenAI Chat Completions 的 API 地址、Key 和模型名。生产部署时建议通过环境变量配置密钥，Web UI 中填写的密钥仅随本次上传请求提交给当前服务端。
...
| 字段 | 说明 |
| --- | --- |
| `data_file` | 业务数据文件，支持 Excel/CSV/JSON/TXT |
| `raw_data_text` | 粘贴的表格文本、CSV、JSON 或导出片段 |
| `request` | 自然语言调度需求 |
| `use_data_agent` | 设置为 `true` 时启用数据接入 Agent |
| `data_agent_api_key` | 可选，留空时读取环境变量 |
| `data_agent_base_url` | 可选，兼容 OpenAI API 的服务地址 |
| `data_agent_model` | 可选，默认 `gpt-4.1-mini` 或环境变量值 |
...
- `data_file`：业务数据文件，由数据接入 Agent 自动识别。
