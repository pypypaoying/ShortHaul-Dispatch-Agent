...
- Web UI：支持加载示例场景、上传本地 CSV/JSON、修改约束和目标、运行优化、查看 KPI 与调度甘特图。
...
5. 直接上传 `payload.json`，或上传 `fleets.csv`、`routes.csv`、`forecast.csv` 等标准文件运行优化。
6. 调用 `/schedule` 或 `/schedule/upload` 运行优化。
...
第一种是通过 Web UI 上传本地文件，适合人工演示、业务验证和小规模调度试跑。页面支持上传完整 `payload.json`，也支持上传 CSV 文件组。

第二种是直接构造 `/schedule` JSON payload，适合服务端系统集成。payload 的核心结构是：
...
第三种是准备 CSV 文件，再用 CLI 转成 API payload，适合从 Excel、TMS 或数据库导出数据：
